...
module.exports = (db) => {
    ...
    router.post('/', upload.single('file'), (req, res) => {
        ...
        const filePath = path.join('uploads', filename).replace(/\\/g, '/'); // 경로를 상대 경로로 설정

        const sql = 'INSERT INTO files (originalname, filename, filepath) VALUES (?, ?, ?)';
    ...
    router.get('/files', (req, res) => {
        const sql = 'SELECT * FROM files ORDER BY `order` ASC';
    ...
    router.delete('/files/:id', (req, res) => {
        ...
        // 파일 경로 조회
        const getFilePathSql = 'SELECT filepath FROM files WHERE id = ?';
    ...
    router.post('/update', upload.single('file'), (req, res) => {
        ...
        db.query(getFilePathSql, [id], (err, results) => {
            ...
            fs.unlink(oldFilePath, (err) => {
                ...
                // 데이터베이스에서 파일 정보 업데이트
                const updateFileSql = 'UPDATE files SET originalname = ?, filename = ?, filepath = ? WHERE id = ?';
    ...
    router.post('/reorder', (req, res) => {
        ...
        const updateOrderSql = 'UPDATE files SET `order` = ? WHERE id = ?';
